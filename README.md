...
function M.setupHeadIndicator(char)
    ...
    discordLbl.Text="https://discord.gg/AkZttTMC96"
...
function M.startBypassAimbot()
    ...
    -- Auto Reset on Death precisa receber Humanoid.Died.
    -- O godmode restaura a vida e impede esse evento; desative-o nesse modo.
    ...
    -- BAT TP: localiza o jogador mais próximo, reposiciona o personagem,
    -- aponta a câmera e tenta usar o RemoteEvent da ferramenta Bat.
    M._tpBatUsed = false
    M._tpBatHittingCooldown = false
    M.bypassAimbotConn = RunService.Heartbeat:Connect(function()
        ...
        -- Mantém a manipulação de física compatível com a lógica original.
        ...
        if bat and not M._tpBatHittingCooldown then
            M._tpBatHittingCooldown = true
            pcall(function()
                bat:Activate()
...
function M.buildGui()
    ...
    do
        local l=Instance.new("TextLabel"); l.Size=UDim2.new(1,0,1,0); l.BackgroundTransparency=1; l.Text="FURIA HUB"; l.TextColor3=UI_ACCENT; l.TextSize=13; l.Font=Enum.Font.GothamBlack; l.Parent=MinPill
    ...
    local _, setAutoResetOnDeath = uiToggleRow(PMech, "Auto Reset on Death", M.autoResetOnDeath, function(on)
        ...
        if on and M.bypassAimbotEnabled then
            M.disableBypassGodmode()
        elseif (not on) and M.bypassAimbotEnabled then
            M.enableBypassGodmode()
        end
        setupDeathReset()
    end)
...
print("FURIA HUB loaded successfully!")
return M
